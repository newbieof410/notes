...
## 项目结构
可以在 [仓库](https://github.com/newbieof410/dockerize-flask-celery) 中找到完整代码.
```
├── app
│   ├── celery_worker
│   │   ├── __init__.py
...
└── requirements.txt
```
项目结构参考 [The Flask Mega-Tutorial](https://blog.miguelgrinberg.com/post/the-flask-mega-tutorial-part-xv-a-better-application-structure).
...
```
这样一来, 在初始化 Celery 前, 必须有一个 Flask 对象. 如果 Flask 对象是直接初始化的, 那么这么做是没有问题的, 只要在 Flask 初始化后, 再初始化 Celery 就好了. 但是如果我们采用工厂模式创建 Flask 对象, 就会出现循环引用的问题.
```
flask_app --> some_blueprint --> celery_app --> flask_app
```
在工厂中创建 `flask_app`, 接着注册 `blueprint`, 在某些 `blueprint` 中可能调用了 Celery
任务, 在任务的定义模块中又引用了 Celery 的实例 `celery_app`, 而初始化这个实例又返回去要依赖 Flask 的实例.
...
```
把定义任务的模块写在末尾, 是为了任务能够正确注册, 同时也避免了循环引用. 在 Celery 启动后, 从输出信息中可以看到任务是否成功注册.
```
[tasks]
...
```
Docker 镜像采用分层的方式构建, 文件中只有 `RUN`, `COPY` 和 `ADD` 命令都会在基础镜像上叠加一个新的镜像层, 其它命令只会产生一层临时镜像, 并且不会直接增加镜像大小. 此外, 镜像的构建过程由上到下展开, 在构建过程中, Docker 会把中间的镜像缓存起来, 方便重用. 因此, 把最常用, 最不易变化的指令, 放在 Dockerfile 最上面的位置可以缩短镜像构建时间.

根据这个原则, 在上面的 Dockerfile 中, 就先安装了 `pip` 升级, 又安装了 `gunicorn` 作为 `WSGI` 服务器, 然后才是构建应用的命令.
...
### ENTRYPOINT & CMD
在 `ENTRYPOINT` 中通常指定容器运行后默认执行的程序, 而在 `CMD` 中指定可变的参数.
...
services:
  ...
  app:
    ...
    depends_on:
      - rabbit
```
整个应用由 3 个服务组成, 分别是 `RabbitMQ`, 执行后台任务的服务和 `Flask` 应用.

一般在服务定义中至少要包含 `image` 或 `build` 两个配置中的一个. 使用 `image` 会从现有的镜像中构建服务, 而使用 `build` 则会根据指定的上下文构建一个镜像.

一个有趣的地方是, 上面构建 `app` 和 `worker` 的是同一个 `Dockerfile`. 首先, worker 的定义和 app 位于同一个项目中. worker 的任务是从消息队列中获取任务, 执行任务; app 则要调用任务, 是任务的发布者, 会把任务添加到消息队列中.
